/*
   ...
   This started life when I was trying to get BlackThunder's pacman source to
   compile and run on GBDK 2.95-3. Once I started getting bits of it sorted,
   I suddenly found myself tearing chunks of it to bits and making modifications.

   With the exception of the basic outline of the program, I don't think that anything
   much of the original exists now. :-)

   History
   =======

   Feb 07/01  Gave up trying to persuade sprintf to work as documented. Decided to upload the program
              as-is to the web and have done with it. Time to start a proper project now, and to look
              into things like vblanks etc. :-)
   Feb 03/01  Added the code to deal with 'try again' and 'game over' displays. Starting dealing
              with score display.    
   Jan 24/01  After a little break, came back to start looking at adding a score & level screen
   Jan 13/01  Got palettes for sprites and tiles working (thanks Shen Mansell & Dave Mariner)
              Added other 3 level maps, added intermediate sprite frames
              Broke the single file of code into a few smaller units
   Jan 12/01  Did some attempted optimisation, unrolled a few loops etc. Dunno if it helps!
   Jan 08/01  Got level-init code sorted (dot counting etc)
              Made contact of ghosts independant, added cherry handling etc
              Added levmap mask array to avoid value-testing background all the time..
   Dec 28/00  Rewrote some chunks of the code & added a load of defines to make
              it easier to see what was doing what.
   Dec ??/00  Started playing with BlackThunder source and GDBK 2.95-3
   ...
   Hiscore table
   Speed / difficulty level setting
   Ghost intelligence needs improving
   Serious testing!
