...
-- which frame better to test, the previous one or the upcoming one?
-- 0,1,2,3,4
  if mmy.active and (p.frame<=4) then
    check_coll(p.oldx, p.oldy, mmy)
  end
-- 4,5,6,7
  if mmy.active and (p.frame>=4) then
    check_coll(p.x, p.y, mmy)
  end
end


function check_coll(textx,testy,mmy)
...
function draw_instr()
  ...
  if (instr_pg>instr_pc) instr_pg=1 -- just in case
  if instr_pg==1 then
    draw_instrp_01()
  ...
  if btnp(5,0) then
    if (high_sl==#highh) then
      ...
      if high_sind==1 then -- space
        add_hname(" ")
      elseif high_sind==2 then -- cursor left
        if (high_cp>0) then
          high_cp-=1
        else
         bad_highs()
        end
      elseif high_sind==3 then -- cursor right
...
function press_x()
  cnt_txt("press ❎ ",120,15) -- extra space to overcome extra width of special 'x' char confusing measuring
end
...
function choose_mmy_tracker()
   return (game_diff>1) -- maps 1,2,3,4, no trackers on lowest level
end
...
function choose_mmy_speed(tracker)
  ...
  if tracker then
    if (game_diff>2) speed=max(speed+0.5,1.5) -- max out at 1.5
  end
  return speed
end
